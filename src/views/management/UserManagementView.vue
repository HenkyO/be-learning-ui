<template>
  <div class="space-y-8 animate-fade-in-up pb-12 max-w-[1400px] mx-auto px-4 sm:px-6 mt-4">
    <div class="bg-white p-8 sm:p-10 rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/40 relative overflow-hidden">
      ...
      <div class="flex flex-col md:flex-row md:items-center gap-6 mb-10 relative z-10 border-b border-slate-100 pb-8">
        <div class="w-16 h-16 bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl flex items-center justify-center text-white shadow-xl">
      ...
      <div class="overflow-x-auto relative z-10 border border-slate-200 rounded-2xl shadow-sm">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="text-sm font-medium text-slate-700 bg-white divide-y divide-slate-100">
...
const openProgressModal = async (user: any) => {
  ...
  } catch (error) { console.error(error) } finally { loadingProgress.value = false }
}
...
const resetAttempts = async (progressId: string, moduleTitle: string) => {
  ...
  try {
    const { error } = await supabase
      .from('user_progress')
      .update({ attempts: 0, status: 'Sedang Dipelajari' }) // Reset status juga jika sebelumnya Selesai/Terkunci
      .eq('id', progressId)
    ...
    if (p) { p.attempts = 0; p.status = 'Sedang Dipelajari' }
    
  } catch (error: any) { alert('Galat mereset: ' + error.message) } finally { isResetting.value = false }
}
